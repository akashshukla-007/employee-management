...
const exportLabels = {
  ...
  date_of_exit:'DATE OF EXIT', reason_exit:'REASON FOR EXIT',
...
async function api(url, options = {}) {
  ...
  catch (netErr) { throw new Error('Network error: could not reach the server. Check your connection and try again.'); }
  ...
  // Non-JSON response: usually means the request never reached our code at all
  // (e.g. the hosting platform itself rejected an oversized upload before our
  // own size-limit check could run, or a proxy/timeout error page). Surface
  // the real status and a snippet of the body instead of a generic message.
  const text = await r.text();
  if (!r.ok) {
    const hint = r.status === 413 ? ' The file is likely too large for the server to accept.' : r.status === 504 ? ' The request took too long to process (timeout).' : '';
    throw new Error(`Request failed (HTTP ${r.status}).${hint}${text ? ' ' + text.slice(0,200) : ''}`);
  }
  return text;
}
...
async function saveEmployee(ev){
  ...
  try{
    ...
    for(const type of ['photo','signature']){
      ...
      if(f){ const data=await compressImageFile(f); await api('/api/upload',{method:'POST',body:JSON.stringify({employeeId:emp.employee_id,type,data,mimeType:'image/jpeg'})}); }
    }
...
function loadImageEl(file){return new Promise((res,rej)=>{const img=new Image();const url=URL.createObjectURL(file);img.onload=()=>{URL.revokeObjectURL(url);res(img)};img.onerror=()=>{URL.revokeObjectURL(url);rej(new Error('Could not read image file.'))};img.src=url})}
// Phone camera photos are often several MB; embedding many of those in an
// Excel export/import (base64) can exceed the hosting platform's request
// size limit. Downscaling to a sensible max dimension keeps files small
// while remaining plenty sharp for an ID photo or signature.
async function compressImageFile(file, maxDim=900, quality=0.85){
  if(!file.type || !file.type.startsWith('image/')) return toDataUrl(file);
  try{
  ...
  }catch(e){ return toDataUrl(file); } // if anything goes wrong, fall back to the original file rather than blocking the save
}
...
async function confirmCapture(){
  ...
  const maxDim = cameraKind==='document' ? 1600 : 900; // documents need more resolution to stay legible
  let out=src;
  if(Math.max(src.width,src.height)>maxDim){
    const scale=maxDim/Math.max(src.width,src.height);
    out=document.createElement('canvas'); out.width=Math.round(src.width*scale); out.height=Math.round(src.height*scale);
...
async function importExcel(file){
  ...
  const CHUNK_SIZE = 2 * 1024 * 1024; // 2MB raw per chunk, safely under the platform's per-request limit regardless of overall file size
  const uploadId = (crypto.randomUUID ? crypto.randomUUID() : 'u'+Date.now()+Math.random().toString(36).slice(2)).replace(/[^a-zA-Z0-9_-]/g,'');
  const totalChunks = Math.max(1, Math.ceil(file.size / CHUNK_SIZE));
  try{
    for(let i=0;i<totalChunks;i++){
      toast(`Uploading part ${i+1} of ${totalChunks}...`,'info');
      const slice = file.slice(i*CHUNK_SIZE, (i+1)*CHUNK_SIZE);
      const dataUrl = await toDataUrl(slice);
      const base64 = dataUrl.split(',')[1] || '';
      await api('/api/import-chunk',{method:'POST',body:JSON.stringify({uploadId, chunkIndex:i, chunk:base64})});
    }
    toast('Processing workbook...','info');
    const result=await api('/api/import-finalize',{method:'POST',body:JSON.stringify({uploadId, totalChunks})});
    await load();show('employees');
    const failed=result.employeesFailed||0;
    toast(`Import complete · ${result.employeesImported} employees imported${failed?`, ${failed} failed`:''}, ${result.photosImported} photos, ${result.signaturesImported} signatures${result.imageFailures?`, ${result.imageFailures} image failures`:''}`, failed?'error':'success');
    if(result.rowErrors?.length){
      alert('These employees could not be imported:\n\n'+result.rowErrors.map(e=>`${e.employeeId}: ${e.error}`).join('\n'));
    }
  }catch(e){toast(e.message,'error')}
}
