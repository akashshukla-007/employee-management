...
function updateDash(){
  ...
  $('deptBreakdown').innerHTML=sorted.length?sorted.map(([code,count])=>`<div class="border rounded-xl p-3 flex items-center justify-between"><span class="font-semibold text-sm">${esc(code)}</span><span class="text-sm text-slate-500">${count} employee${count===1?'':'s'}</span></div>`).join(''):'<p class="text-sm text-slate-500">No site codes recorded yet.</p>';
...
const cameraPendingData={photo:null, signature:null};
