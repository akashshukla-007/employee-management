...
const exportLabels = {
  ...
  date_of_exit:'DATE OF EXIT', reason_exit:'REASON OF EXIT',
...
let deleteInFlight=new Set();
async function deleteEmployee(id){
  const emp=employees.find(x=>x.id===id);
  if(!confirm(`Are you sure you want to delete ${emp?fullName(emp):'this employee'} and associated records?`))return;
  if(deleteInFlight.has(id))return; // guard against double-click / duplicate requests
  deleteInFlight.add(id);
  try{await api('/api/employees?id='+encodeURIComponent(id),{method:'DELETE'});toast('Employee deleted.','success');await load()}
  catch(e){toast(e.message,'error')}
  finally{deleteInFlight.delete(id)}
}
