...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'DELETE') {
      const id = req.query.id;
      if (!id) return res.status(400).json({ error: 'Employee UUID is required.' });
      // maybeSingle() (not single()) so a row that's already gone (e.g. a
      // duplicate/double-clicked delete request) is treated as "already
      // deleted" instead of throwing "Cannot coerce the result to a single
      // JSON object".
      const { data: emp, error: findError } = await db.from('employees').select('photo_path,signature_path').eq('id', id).maybeSingle();
      if (findError) throw findError;
      if (!emp) return res.status(200).json({ ok: true, alreadyDeleted: true });
